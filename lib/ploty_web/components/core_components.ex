defmodule PlotyWeb.CoreComponents do
  @moduledoc """
  ...
  attr :histfunc, :string, default: "count", values: ~w(count sum avg min max)
  ...
  def histogram(assigns) do
    ...
    <div
      id={"histogram-#{@id}"}
      phx-hook="histogram"
      data-trace={Jason.encode!(%{x: @series, histfunc: @histfunc, name: @name})}
    >
    </div>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(PlotyWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(PlotyWeb.Gettext, "errors", msg, opts)
    end
  end
